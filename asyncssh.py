...
async def check_ssh_credentials(host, port, username, password, config, success_file):
    ...
    max_retries = config['ssh'].get('max_retries', 1)
    ...
    for attempt in range(max_retries):
        ...
        except (asyncssh.ConnectionError, socket.timeout) as e:
            result['message'] = f'Connection error: {e}'
            with print_lock:
                print(f"{Colors.ERROR}Error{Colors.ENDC} - {host}:{port} - {username} - {result['message']}")
            logging.error(f"Connection error for {host}:{port} with {username} - {e}")
            await asyncio.sleep(2)  # Delay before retrying
